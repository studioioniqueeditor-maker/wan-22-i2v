...
# Register API blueprint with separate rate limiter
api_limiter = get_api_rate_limiter()
app.register_blueprint(api_bp)
# Apply the API-specific rate limiter to the blueprint
...
def log_request_info():
    ...
    # Don't log sensitive data for API requests
    if not request.path.startswith('/api/') and request.method == 'POST':
...
def _run_video_generation(user_id, file, form_data):
    """Helper function to handle the core video generation logic (legacy web interface)."""
...
# --- Application Routes (Web Interface) ---
...
@login_required
def generate():
    """Web interface generation - synchronous for immediate feedback."""
    user_id = session.get('user_id')
...
@login_required
def get_history_api():
    """Legacy API endpoint for history - uses Supabase"""
    user_id = session.get('user_id')
